...
- **三种 Prompt 模式** — 只要答案（默认）/ 答案 + 解析 / 自定义
- **可拖动悬浮气泡**，可设透明度（0.4–1.0），ESC 隐藏，⚙ 跳设置
...
| --- | --- | --- |
| 只要答案（默认） | 严格 JSON `{answer:"..."}` | 只有「答案」 |
| 答案 + 解析 | 严格 JSON `{answer, reasoning}` | 「答案」+「解析」 |
| 自定义 | 你写的文本 | 按模式判定 |
...
| `Ctrl+Shift+P` | **抓取整页 DOM 文本** | 把整页 `innerText` 前 6000 字符发给 LLM。适合：长文章、整段题文、多选合集。 |
